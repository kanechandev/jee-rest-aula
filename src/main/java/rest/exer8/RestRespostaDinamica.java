...
public class RestRespostaDinamica {
	...
	public Response teste(@QueryParam("numero") @DefaultValue("1") Integer numero) {
		...
		f.setNome("João");		
		f.setCpf(44214295412L);
		f.setSalario(BigDecimal.TEN);		
		if(numero == 2) {
			return Response.ok()
					.type(MediaType.APPLICATION_XML).entity(f).build();
		}
		if(numero == 3) {
			return Response.ok()
					.type(MediaType.APPLICATION_JSON).entity(f).build();
		}
		return Response.ok()
				.type(MediaType.TEXT_PLAIN).entity("Código não tratado").build();
	}
}
